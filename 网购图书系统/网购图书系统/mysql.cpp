...
void OprDB::vtomysql()
{
	...
	{
		sprintf_s(buf1,"%s'%s','%s','%s',%lf,%d)",buf2,iter1->m_name,iter1->m_author,iter1->m_publish,iter1->m_price,iter1->m_amount);
...
//		cout<<buf1<<endl;
	...
	{		
		sprintf_s(buf3,"%s%d,'%s','%s','%s','%s','%s','%s')",buf4,iter2->m_id,iter2->m_name,iter2->m_passwd,iter2->m_lev,iter2->m_tel,iter2->m_adress,iter2->m_time);
		mysql_query(m_mysql,buf3);
	sprintf_s(buf7,"%s%d",buf8,id);
...
//		cout<<buf3<<endl;
	...
	{
		sprintf_s(buf5,"%s%d,'%s','%s',%d,%lf,%lf,%lf,%d,'%s')",buf6,iter3->m_num,iter3->m_name,iter3->m_author,iter3->m_amount,iter3->m_dst,iter3->m_postage,iter3->m_total,iter3->m_id,iter3->m_remark);
